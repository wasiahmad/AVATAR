...
from graphcodebert.model import Seq2Seq
...
for lang in dfg_function:
    LANGUAGE = Language(COMPILED_SO_FILE, lang)
    parser = Parser()
...
def read_examples(data_dir, source, target, part):
    ...
    with open(src_file_name, encoding='utf8') as f1, \
        ...
        for src, tgt in zip(f1, f2):
            examples.append(
                Example(source=src.strip(), target=tgt.strip(), src_lang=source, tgt_lang=target)
            )
    return examples
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(tqdm(examples, total=len(examples))):
        ## extract data flow
        code_tokens, dfg = extract_dataflow(example.source, parsers[example.src_lang], example.src_lang)
...
def main():
    ...
    if args.load_model_path is not None:
        logger.info("loading model from {}".format(args.load_model_path))
