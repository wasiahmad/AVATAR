...
class Evaluator(object):
    ...
    def run_all_evals(self, trainer, eval_splits=EVAL_DATASET_SPLITS):
        """
        Run all evaluations.
        """
        params = self.params
    ...
    def evaluate_mt(
        ...
        for deobfuscation_proba in (
            ...
            if eval_bleu and data_set in datasets_for_bleu:
                # evaluate BLEU score
                bleu = eval_codexglue_bleu(ref_path, hyp_paths[0])
...
def eval_codexglue_bleu(ref, hyp):
    """
    Given a file of hypothesis and reference files,
    evaluate the BLEU score using Moses scripts.
    """
    ...
    translations = []
    with open(hyp, 'r', encoding='utf-8') as fh:
        for line in fh:
