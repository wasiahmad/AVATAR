...
# from evaluation_scripts import text_evaluator
# from evaluation_scripts.CodeBLEU import bleu
# from evaluation_scripts.CodeBLEU import calc_code_bleu
...
class TranslationMultiSimpleEpochTaskExtended(TranslationMultiSimpleEpochTask):
    ...
    def _inference_with_bleu(self, generator, sample, model):
        ...
        # if self.args.bleu_type == 'code':
        #     # calculate ngram match (BLEU)
        #     lids = [self.target_dictionary[l] for l in target[:, 0].tolist()]
        #     tokenized_hyps = [calc_code_bleu.language_specific_processing(x.split(), lids[i])
        #                       for i, x in enumerate(hyps)]
        #     tokenized_refs = [[calc_code_bleu.language_specific_processing(r.split(), lids[i])]
        #                       for i, r in enumerate(refs)]
        #     return bleu.corpus_bleu(tokenized_refs, tokenized_hyps)
        # else:
        #     return text_evaluator.bleuFromLists(hyps, refs)[0]
