...
function train() {
    ...
    SAVE_DIR=${CURRENT_DIR}/transformer/${DATA_SRC}/${SOURCE}2${TARGET}
    mkdir -p $SAVE_DIR
...
function program_translation_ngram_evaluation() {
    SAVE_DIR=${CURRENT_DIR}/transformer/program/${SOURCE}2${TARGET}
...
function function_translation_ngram_evaluation() {
    SAVE_DIR=${CURRENT_DIR}/transformer/function/${SOURCE}2${TARGET}
    MODEL_PATH=${SAVE_DIR}/checkpoint_best.pt
